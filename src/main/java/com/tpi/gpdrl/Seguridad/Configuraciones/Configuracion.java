    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity, AuthenticationManager authenticationManager) throws Exception {
        ...
        return httpSecurity
                ...
                .authorizeHttpRequests(http -> {
                    http.requestMatchers("/css/**", "/js/**", "/image/**","/service-worker.js", "/manifest.json").permitAll();

                    // LOGIN
                    http.requestMatchers(HttpMethod.POST, "/login").anonymous(); // Para enviar la petición de ingreso

                    // RECUPERAR CONTRASEÑA
                    http.requestMatchers(HttpMethod.GET, "/recuperarContra").anonymous();
                    http.requestMatchers(HttpMethod.POST, "/seguridad/recuperar").anonymous();

                    // CREAR CLIENTE
                    http.requestMatchers(HttpMethod.GET, "/Cliente/CrearCuenta").anonymous();
                    http.requestMatchers(HttpMethod.POST, "/Cliente/guardarCliente").anonymous();
                    // http.requestMatchers(HttpMethod.POST, "/Cliente/actualizarCantidad").permitAll();


                    // LOGOUT
                    http.requestMatchers(HttpMethod.GET, "/logout-success").anonymous(); // Para enviar la petición de nueva empresa
            ...
            protected String determineTargetUrl(Authentication authentication) {
                ...
                for (GrantedAuthority authority : authorities) {
                    if (authority.getAuthority().equals("ROLE_CLIENTE")) {
                        return "/Cliente/RealizarPedido"; // URL para usuarios con rol ADMIN
                    }
                    if (authority.getAuthority().equals("ROLE_REPARTIDOR")) {
                        return "/repartidor"; // URL para usuarios con rol ADMIN
                    }
                }
